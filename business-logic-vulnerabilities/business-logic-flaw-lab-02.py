...
def is_add_negative_to_cart_successful(url, session):
    cart_url = url + "/cart"
    data_cart = {
        "productId": "20",
        "redir": "PRODUCT",
...
def is_add_positive_to_cart_successful(url, session):
    cart_url = url + "/cart"
    data_cart = {
        "productId": "1",
        "redir": "PRODUCT",
...
if __name__ == "__main__":
    try:
        ...
        if not is_login_successful(url, session):
            print("[-] Login Failed.")
            sys.exit(FAIL)
        if not is_add_negative_to_cart_successful(url, session):
            print("[-] Add negative to Cart Failed.")
            sys.exit(FAIL)
        if not is_add_positive_to_cart_successful(url, session):
            print("[-] Add positive to Cart Failed.")
            sys.exit(FAIL)
